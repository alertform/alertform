...
| [chrome-hands](https://github.com/alertform/chrome-hands) | A browser agent that gives Claude real hands in Chrome: MV3 extension + local bridge, driven from a Bash CLI. Trusted input via CDP, domain allowlist and confirm-before-send guardrails, one tab group per caller so concurrent agents never collide. | JavaScript · CDP · MV3 |
